...
function CommentItem(props) {
  ...
  function onFinish() {
    ...
    window.app.api.editComment(comment.id, detail).then((res) => {
  ...
  function onDelete() {
    window.app.api.deleteComment(comment.id).then((res) => {
    ...
    <div className="comment-list">
      ...
      {comments.length > 0 &&
        comments.map(comment => {
          return (
            <CommentItem comment={comment} novel={props.novel} key={comment.id}/>
          );
        })
      }
    </div>
